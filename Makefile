...
DESIGN_HDRS = $(addprefix $(BSG_IP_CORES)/, bsg_misc/bsg_defines.v bsg_noc/bsg_noc_pkg.v)                          \
              ...
              $(addprefix $(MODULES)/,bsg_vscale_pkg.v)

DESIGN_SRCS = \
  $(addprefix $(BSG_IP_CORES)/, bsg_misc/bsg_transpose.v bsg_misc/bsg_crossbar_o_by_i.v           \
  ...
  $(addprefix $(MODULES)/, bsg_vscale_core.v bsg_vscale_tile.v bsg_vscale_tile_array.v)           \
  $(addprefix $(TEST_DIR)/, common/v/bsg_manycore_spmd_loader.v)

$(MEM_DIR)/bin/%.bin:
	mkdir -p $(MEM_DIR)/hex $(MEM_DIR)/bin $(MEM_DIR)/rom
	cp $(subst _,-,$(VSCALE_INPUTS)/$*.hex) $(MEM_DIR)/hex
...
instr_regress: $(foreach x, $(subst -,_,$(RV32_TESTS)), $(x).run)

%.run: $(ROM_DIR)/bsg_rom_%.v
	$(VLOG) $(DESIGN_HDRS) $(DESIGN_SRCS) $(ROM_DIR)/bsg_rom_$*.v $(SIM_TOP_DIR)/test_bsg_vscale_tile_array.v -d SPMD=$*
	$(VELAB) test_bsg_vscale_tile_array | grep -v Compiling
	$(VSIM)
