...
set BSG_LINK_SDR_USE_GENERATED_CLOCK 1
...
proc bsg_link_sdr_constraints { \
  ...
  # token
  set in_tkn_clk_period         [expr 2*$out_clk_period]
  ...
  set out_tkn_clk_period         [expr 2*$in_clk_period]
  ...
  if {[get_ports -quiet $in_clk_port] != ""} {
    set_driving_cell -no_design_rule -lib_cell "CKBD4BWP7T35P140LVT" $in_clk_port
  }
  if {[get_ports -quiet $in_dv_port] != ""} {
    set_driving_cell -no_design_rule -lib_cell "CKBD4BWP7T35P140LVT" $in_dv_port
  }
  ...
  if {[get_ports -quiet $out_clk_port] != ""} {
    set_load [load_of [get_lib_pin "*/CKBD4BWP7T35P140LVT/I"]] $out_clk_port
  }
  if {[get_ports -quiet $out_dv_port] != ""} {
    set_load [load_of [get_lib_pin "*/CKBD4BWP7T35P140LVT/I"]] $out_dv_port
  }
}
