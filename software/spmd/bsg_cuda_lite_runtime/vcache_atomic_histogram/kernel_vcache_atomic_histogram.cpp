...
void validate(unsigned *rcode)
{
  for (int i = 0; i < 32; i++)
    if (answers[i] != histogram[i]) {
      bsg_print_int(i);
      *rcode = -1u;
    }
}
...
int kernel_vcache_atomic_histogram(unsigned *rcode)
{

  // Uncomment this block to serialize do_histogram_work() 
  // barrier.sync(); 
  /* if (__bsg_id == 0) do_histogram_work(); */
  // barrier.sync(); 
  /* if (__bsg_id == 1) do_histogram_work(); */
  // barrier.sync(); 
  /* if (__bsg_id == 2) do_histogram_work(); */
  // barrier.sync(); 
  /* if (__bsg_id == 3) do_histogram_work(); */

  // Uncomment this block to parallelize do_histogram_work()
  barrier.sync();
  do_histogram_work();
  
  barrier.sync();

  if (__bsg_id == 0) validate(rcode);
