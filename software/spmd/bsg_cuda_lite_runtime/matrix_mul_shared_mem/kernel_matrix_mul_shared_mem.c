...
void __attribute__ ((noinline)) subblock2shmem (int *A, int *sh_dest, int M, int N, int block_size_y, int block_size_x, int sub_block_y, int sub_block_x) { 
	...
	for (int iter_y = __bsg_y; iter_y < block_size_y; iter_y += bsg_tiles_Y) { 
		for (int iter_x = __bsg_x; iter_x < block_size_x; iter_x += bsg_tiles_X) { 
			...
			bsg_tilegroup_store (sh_dest, (iter_y * block_size_x + iter_x), A[((iter_y + start_y) * N + iter_x + start_x)]);
		}
	}
	return; 
}
...
void __attribute__ ((noinline)) shmem2subblock (int *A, int *sh_src, int M, int N, int block_size_y, int block_size_x, int sub_block_y, int sub_block_x) { 
	...
	for (int iter_y = __bsg_y; iter_y < block_size_y; iter_y += bsg_tiles_Y) { 
		for (int iter_x = __bsg_x; iter_x < block_size_x; iter_x += bsg_tiles_X) { 
			...
			bsg_tilegroup_load (sh_src, (iter_y * block_size_x + iter_x), A[((iter_y + start_y) * N + iter_x + start_x)]);
		}
	}
	return; 
}
	...
	for (int iter_y = __bsg_y; iter_y < block_size_y; iter_y += bsg_tiles_Y) { 
		for (int iter_x = __bsg_x; iter_x < block_size_x; iter_x += bsg_tiles_X) { 
			...
			for (int k = 0; k < BLOCK_WIDTH; k ++) { 
				...
				bsg_tilegroup_load (sh_A, (iter_y * BLOCK_WIDTH + k), lc_A); 
				// lc_B <-- sh_B[iter_y][iter_x]	remember B is transposed
				bsg_tilegroup_load (sh_B, (iter_x * BLOCK_WIDTH + k), lc_B);
				sum += lc_A * lc_B;
			}

			if (!block_num) { 
				// sh_C[iter_y][iter_x] <-- sum
				bsg_tilegroup_store (sh_C, (iter_y * block_size_x + iter_x), sum);
			}
			else { 
				int lc_C;
				// sh_C[iter_y][iter_x] += sum
				bsg_tilegroup_load (sh_C, (iter_y * block_size_x + iter_x), lc_C);
				bsg_tilegroup_store (sh_C, (iter_y * block_size_x + iter_x), lc_C + sum);
			} 
		}
	}
	return;
