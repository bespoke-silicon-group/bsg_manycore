...
void compute_block(float* bsg_attr_noalias sp_dest,
                   float* bsg_attr_noalias sp_mat1,
                   float* bsg_attr_noalias sp_mat2) {
        // TODO: Assert when BX%SBX != 0
        // TODO: Assert when BY%SBY != 0
        //for (int iii = 0; iii < BLOCK_DIM; iii += 4) {
        for (int by_i = 0; by_i < BY; by_i += SBY) {
                //for(int jjj = 0; jjj < BLOCK_DIM; jjj += 4) {
                for (int bx_i = 0; bx_i < BX; bx_i += SBX) {
                        // Load in a sub-block result, with corner by_i, bx_i
                        // int sp_dest_base = iii * BLOCK_DIM + jjj;
                        int sb_corner = by_i * BY + bx_i;
                        float res[SBY][SBX];
                        for(int sby_i = 0; sby_i < SBY; ++sby_i){
                                for(int sbx_i = 0; sbx_i < SBX; ++sbx_i){
                                        res[sby_i][sbx_i] = sp_dest[sb_corner + sby_i * BX + sbx_i];
                                }
                        }
                        /*
                        register float res00 = sp_dest[sp_dest_base + 0             + 0];
                        ...
                        register float res33 = sp_dest[sp_dest_base + 3 * BLOCK_DIM + 3];
                        */
                        for(int sbz_i = 0; sbz_i < BY; ++sbz_i) {
                                ///for(int kkk = 0; kkk < BLOCK_DIM; kkk++) {

                                // Load a sub-column of mat 1
                                int mat1_corner = sbz_i + by_i * BX;
                                int mat2_corner = sbz_i * BY + bx_i;
                                float col[SBY];
                                float row[SBY];
                                // NB: Ideally, mat1 would be loaded
                                // transposed, so that this can be
                                // simplified with a reused index and unique offset.
                                for(int i = 0; i < SBY; ++i){
                                    col[i] = sp_mat1[mat1_corner + i * BX];
                                    row[i] = sp_mat2[mat2_corner + i];
                                }

                                // Compute SBY*SBX partial sums
                                for(int sby_i = 0; sby_i < SBY; ++sby_i){
                                        // Scalar - Vector Product
                                        for(int sbx_i = 0; sbx_i < SBX; ++sbx_i){
                                                res[sby_i][sbx_i] += col[sby_i] * row[sbx_i];
                                        }
                                }
                                /*
                                int mat1_base = kkk + iii * BLOCK_DIM;
                        ...
                        for(int sby_i = 0; sby_i < SBY; ++sby_i){
                                for(int sbx_i = 0; sbx_i < SBX; ++sbx_i){
                                        sp_dest[sb_corner + sby_i * BX + sbx_i] = res[sby_i][sbx_i];
                                }
                        }
                }
        }
